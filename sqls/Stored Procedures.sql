    ...
    IF p_password = password_text THEN
		...
		IF order_exists > 0 THEN
			...
			IF p_status IN ('Pending', 'Processing', 'Shipped', 'Delivered') THEN
				...
				WHERE OrderID = p_order_id and productID = p_product_id;
			ELSE
				SIGNAL SQLSTATE '45000'
...
CREATE PROCEDURE DeleteDiscountsForProduct(
    IN p_product_id INT,
    IN Discount_amount DECIMAL(10, 2),
    IN p_supplier_id INT,
    IN p_password INT
)
BEGIN
    DECLARE product_exists INT;
    DECLARE password_text INT;
    ...
    IF p_password = password_text THEN
		...
		IF product_exists > 0 THEN
			...
			UPDATE discounts
				SET DiscountAmount = Discount_amount
				WHERE productID=p_product_id;

			SELECT 'Discounts for product deleted.' AS Message;
		ELSE
			SIGNAL SQLSTATE '45000'
			SET MESSAGE_TEXT = 'Product does not exist for the given supplier id';
		END IF;
	ELSE
			-- Raise an error if the product already exists
			SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Given Supplier password is wrong';
